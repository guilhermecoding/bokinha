...
const APP_NAME = 'questions-solved-api';

export async function GET(
    _req: Request,
    {params}: { params: Promise<{ id: string }> }
) {
    log(APP_NAME, 'INFO', 'GET /api/questions/[id]/solved - Iniciando requisição');
    ...
    try {
        ...
        log(APP_NAME, 'INFO', 'Verificando se questão foi resolvida', {userId: user.id, questionId: id});

        const already = await prisma.solvedQuestion.findFirst({
            where: {userId: user.id, questionId: id},
        });

        log(APP_NAME, 'INFO', 'Verificação concluída', {userId: user.id, questionId: id, done: Boolean(already)});

        return NextResponse.json({status: 'ok', done: Boolean(already)}, {status: 200});
    } catch (err) {
        log(APP_NAME, 'ERROR', 'Erro ao verificar se questão foi resolvida', {
